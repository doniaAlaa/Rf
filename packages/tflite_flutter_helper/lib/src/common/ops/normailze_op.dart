...
class NormalizeOp implements TensorOperator {
  ...
  NormalizeOp(double mean, double stddev) {
    ...
    SupportPreconditions.checkArgument(stddev != 0.0, errorMessage: "Stddev cannot be zero.");
  ...
  NormalizeOp.multipleChannels(List<double> mean, List<double> stddev) {
    ...
    SupportPreconditions.checkArgument(mean.length == stddev.length,
        errorMessage: "Per channel normalization requires same number of means and stddevs");
    SupportPreconditions.checkArgument(mean.length > 0, errorMessage: "Means and stddevs are empty.");
    ...
    for (int i = 0; i < numChannels; i++) {
      SupportPreconditions.checkArgument(this.stddev[i] != 0, errorMessage: "Stddev cannot be zero.");
  ...
  TensorBuffer apply(TensorBuffer input) {
    ...
    SupportPreconditions.checkArgument(
        ...
        errorMessage: "Number of means (stddevs) is not same with number of channels (size of last axis).");
