...
class TensorLabel {
  ...
  factory TensorLabel(Map<int, List<String>> axisLabels, TensorBuffer tensorBuffer) {
  ...
  TensorLabel.fromMap(Map<int, List<String>> axisLabels, TensorBuffer tensorBuffer) {
    SupportPreconditions.checkNotNull(axisLabels, message: "Axis labels cannot be null.");
    SupportPreconditions.checkNotNull(tensorBuffer, message: "Tensor Buffer cannot be null.");
    _axisLabels = axisLabels;
    _tensorBuffer = tensorBuffer;
    _shape = tensorBuffer.getShape();

    axisLabels.forEach((axis, labels) {
      SupportPreconditions.checkArgument(axis >= 0 && axis < _shape.length, errorMessage: "Invalid axis id: $axis");
      SupportPreconditions.checkNotNull(labels, message: "Label list is null on axis: $axis");
      SupportPreconditions.checkArgument(_shape[axis] == labels.length,
          errorMessage: "Label number " + "${labels.length} mismatch the shape on axis $axis");
    });
  }
  ...
  ///
  /// [tensorBuffer] to be labeled.
  factory TensorLabel.fromList(List<String> axisLabels, TensorBuffer tensorBuffer) {
    return TensorLabel.fromMap(_makeMap(getFirstAxisWithSizeGreaterThanOne(tensorBuffer), axisLabels), tensorBuffer);
  }
  ...
  Map<String, TensorBuffer> getMapWithTensorBuffer() {
    ...
    SupportPreconditions.checkArgument(_axisLabels.containsKey(labeledAxis),
        errorMessage: "get a <String, TensorBuffer> map requires the labels are set on the first non-1 axis.");
    ...
    SupportPreconditions.checkNotNull(labels, message: "Label list should never be null");
    labels.asMap().forEach((i, label) {
      ...
      labelBuffer.loadBuffer(bData.buffer, shape: _shape.sublist(labeledAxis + 1, _shape.length));
  ...
  Map<String, double> getMapWithFloatValue() {
    ...
    SupportPreconditions.checkState(labeledAxis == _shape.length - 1,
        errorMessage: "get a <String, Scalar> map is only valid when the only labeled axis is the last one.");
    List<String> labels = _axisLabels[labeledAxis]!;
  ...
  List<Category> getCategoryList() {
    int labeledAxis = getFirstAxisWithSizeGreaterThanOne(_tensorBuffer);
    SupportPreconditions.checkState(labeledAxis == _shape.length - 1,
        errorMessage: "get a Category list is only valid when the only labeled axis is the last one.");
    List<String> labels = _axisLabels[labeledAxis]!;
    List<double> data = _tensorBuffer.getDoubleList();
    SupportPreconditions.checkState(labels.length == data.length);
  ...
  static int getFirstAxisWithSizeGreaterThanOne(TensorBuffer tensorBuffer) {
    ...
    throw new ArgumentError("Cannot find an axis to label. A valid axis to label should have size larger than 1.");
  }
