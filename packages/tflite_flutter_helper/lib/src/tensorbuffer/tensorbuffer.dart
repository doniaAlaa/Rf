...
abstract class TensorBuffer {
  ...
  static TensorBuffer createFrom(TensorBuffer buffer, int dataType) {
    SupportPreconditions.checkNotNull(buffer, message: "Cannot create a buffer from null");
    TensorBuffer result;
    if (buffer.isDynamic) {
    ...
    if (buffer.getDataType() == TfLiteType.kTfLiteFloat32 && dataType == TfLiteType.kTfLiteFloat32) {
  ...
  void loadBuffer(ByteBuffer buffer, {List<int>? shape}) {
    SupportPreconditions.checkNotNull(buffer, message: "Byte Buffer cannot be null");

    int flatSize = computeFlatSize(shape ?? this.shape);

    SupportPreconditions.checkArgument((ByteData.view(buffer).lengthInBytes == getTypeSize() * flatSize),
        errorMessage:
    ...
    if (!_isDynamic) {
      SupportPreconditions.checkArgument(flatSize == this.flatSize,
          errorMessage: "The size of byte buffer and the size of the tensor buffer do not match.");
    } else {
  ...
  void _allocateMemory(List<int> shape) {
    SupportPreconditions.checkNotNull(shape, message: 'TensorBuffer shape cannot be null.');
    SupportPreconditions.checkArgument(_isShapeValid(shape), errorMessage: 'TensorBuffer shape cannot be null.');
  ...
  void resize(List<int> shape) {
    if (_isDynamic) {
      ...
      SupportPreconditions.checkArgument((computeFlatSize(shape) == computeFlatSize(this.shape)));
      this.shape = List<int>.from(shape);
    }
  }
