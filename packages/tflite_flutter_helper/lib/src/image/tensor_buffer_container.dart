...
class TensorBufferContainer implements BaseImageContainer {
  ...
  static TensorBufferContainer create(TensorBuffer buffer, ColorSpaceType colorSpaceType) {
    checkArgument(colorSpaceType == ColorSpaceType.RGB || colorSpaceType == ColorSpaceType.GRAYSCALE,
        message: "Only ColorSpaceType.RGB and ColorSpaceType.GRAYSCALE are supported. Use" +
            " `create(TensorBuffer, ImageProperties)` for other color space types.");

    return TensorBufferContainer._(buffer, colorSpaceType, colorSpaceType.getHeight(buffer.getShape()),
        colorSpaceType.getWidth(buffer.getShape()));
  }

  TensorBufferContainer._(TensorBuffer buffer, ColorSpaceType colorSpaceType, int height, int width) {
  ...
  TensorBufferContainer clone() {
    return TensorBufferContainer._(
        TensorBuffer.createFrom(_buffer, _buffer.getDataType()), colorSpaceType, height, width);
  }
  ...
  CameraImage get mediaImage {
    throw UnsupportedError("Converting from TensorBuffer to android.media.Image is unsupported.");
  }
