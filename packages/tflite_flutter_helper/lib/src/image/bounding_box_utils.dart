...
class BoundingBoxUtils {
  ...
  static List<Rect> convert({
    ...
    SupportPreconditions.checkArgument(
      ...
      errorMessage: "Axis $boundingBoxAxis is not in range (-(D+1), D), where D is the number of dimensions of input" +
          " tensor (shape=$shape)",
    );
    ...
    SupportPreconditions.checkArgument(
      valueIndex.length == 4,
      errorMessage: "Bounding box index list length ${valueIndex.length} is not 4. Got index list $valueIndex",
    );
    SupportPreconditions.checkArgument(tensor.getDataType() == TfLiteType.kTfLiteFloat32,
        errorMessage: "Bounding Boxes only create from FLOAT32 buffers. Got: ${tensor.getDataType()}");
    ...
    for (int i = 0; i < a; i++) {
      for (int j = 0; j < b; j++) {
        ...
        boundingBoxList.add(_convertOneBoundingBox(values, boundingBoxType, coordinateType, height, width, valueIndex));
      }
    }
  ...
  static Rect _convertOneBoundingBox(
      List<double> values, BoundingBoxType type, CoordinateType coordinateType, int height, int width,
  ...
  static Rect _convertFromBoundaries(
      List<double> values, CoordinateType coordinateType, int imageHeight, int imageWidth) {
    double left = values[0];
    double top = values[1];
    double right = values[2];
    double bottom = values[3];
    return _getRectF(left, top, right, bottom, imageHeight, imageWidth, coordinateType);
  }

  static Rect _convertFromUpperLeft(
      List<double> values, CoordinateType coordinateType, int imageHeight, int imageWidth) {
    double left = values[0];
    double top = values[1];
  ...
  static Rect _convertFromCenter(List<double> values, CoordinateType coordinateType, int imageHeight, int imageWidth) {
  ...
  static Rect _getRectF(double left, double top, double right, double bottom, int imageHeight, int imageWidth,
    ...
    if (coordinateType == CoordinateType.PIXEL) {
      ...
      return new Rect.fromLTRB(left * imageWidth, top * imageHeight, right * imageWidth, bottom * imageHeight);
    } else {
      throw new ArgumentError("Cannot convert coordinate type " + coordinateType.toString());
    }
  }
}
